...
export async function getHeroInformationPosts(preview) {
  const entries = await fetchGraphQL(
    `query {
      heroInformationPostCollection(preview: ${preview ? "true" : "false"}, limit: 10) {
        items {
          title,
          postsCollection(preview: ${preview ? "true" : "false"}, limit: 10){
            items{
  ...
  return entries?.data?.heroInformationPostCollection?.items[0];
}

export async function getFooterItems(id, preview) {
  const entries = await fetchGraphQL(
    `query {
      footerCollection(where : {sys : { id : "${id}" }}, preview: false, limit: 1) {
        items {
          footerlinksCollection(preview: false, limit: 10){
            items{
              ... on FooterItems{
                title,
                categoryCollection(preview: false, limit: 10){
                  items{
                    ...on NavbarItems{
                      title
                    }
  ...
  return entries?.data?.footerCollection?.items[0].footerlinksCollection;
}